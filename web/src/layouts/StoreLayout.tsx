import { BookOpen, LogIn, Menu, PackageSearch, Search, ShoppingCart, UserRound } from 'lucide-react';
...
export function StoreLayout() {
  ...
  const isStaffOrAdmin = Boolean(user?.roles.some((role) => role === 'Staff' || role === 'Admin'));
  const showShoppingNav = !isStaffOrAdmin;
  ...
  const mobileNavLinks = useMemo(
    () => [
      { to: user ? '/account' : '/login', label: user ? 'Tài khoản của tôi' : 'Đăng nhập', show: true },
      { to: '/cart', label: `Giỏ hàng (${cartCount})`, show: showShoppingNav && isCustomer },
      { to: '/orders', label: 'Đơn hàng của tôi', show: showShoppingNav && isCustomer }
    ],
    [cartCount, isCustomer, showShoppingNav, user]
  );
