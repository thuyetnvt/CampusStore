    ...
    <div className="overflow-x-hidden">
      <section className="border-b border-emerald-100 bg-white">
        <div className="mx-auto max-w-7xl px-4 py-6 sm:py-8 lg:py-10">
          <div className="relative overflow-hidden rounded-lg border border-emerald-100 bg-[#fbfffd]">
            <div className="relative z-10 min-w-0 max-w-full px-5 py-8 sm:px-8 sm:py-10 lg:max-w-2xl lg:px-10 lg:py-14">
              <p className="text-xs font-semibold uppercase tracking-wide text-emerald-700 sm:text-sm">
                Đồ dùng học tập cho sinh viên
              </p>
              <h1 className="mt-3 max-w-[18rem] text-2xl font-bold leading-tight text-slate-950 min-[360px]:max-w-[20rem] min-[360px]:text-3xl sm:max-w-none sm:text-4xl lg:text-5xl">
                Chuẩn bị góc học tập gọn gàng, đủ món, dễ mua
              </h1>
              <p className="mt-4 max-w-[19rem] text-base leading-7 text-slate-700 sm:max-w-none">
            ...
            <div className="grid grid-cols-4 gap-2 px-5 pb-5 sm:px-8 lg:hidden">
            ...
            <div className="absolute bottom-8 right-8 top-8 hidden w-[42%] grid-cols-2 gap-3 lg:grid">
              {heroImages.map((image, index) => (
                <div
                  key={image.src}
                  className={`overflow-hidden rounded-lg bg-slate-100 shadow-sm ${index === 1 ? 'mt-8' : ''} ${
                    index === 2 ? '-mt-8' : ''
                  }`}
                >
            ...
            <div className="pointer-events-none absolute inset-y-0 right-0 hidden w-1/2 bg-gradient-to-r from-transparent to-emerald-50/60 lg:block" />
          </div>
        </div>
      </section>
      ...
      <div className="mx-auto max-w-7xl px-4 py-10">
        <section>
          ...
          {categoriesQuery.isLoading ? (
            ...
            <div className="rounded-md border border-rose-200 bg-rose-50 px-4 py-5 text-sm text-rose-700">
              Không tải được danh mục. Vui lòng kiểm tra backend.
            </div>
          ) : (
        ...
        <ProductSection title="Sản phẩm bán chạy" actionTo="/products?sort=best_selling" query={bestSellerQuery} />
        <ProductSection
          title="Đang giảm giá"
          actionTo="/products"
          query={saleQuery}
        ...
        />
        <ProductSection title="Sản phẩm mới" actionTo="/products?sort=newest" query={newProductsQuery} />
    ...
    <section className="mt-14">
      ...
      {query.isLoading ? (
        ...
        <div className="rounded-md border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-700">
          Không tải được sản phẩm. Vui lòng kiểm tra backend.
        </div>
